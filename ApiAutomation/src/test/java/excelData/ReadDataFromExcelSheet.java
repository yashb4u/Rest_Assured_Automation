...
//		wb.close();
	...
	public static String getCellData(String xlfile,String xlsheet,int rownum,int colnum) throws IOException 
	{
		...
		try 
		{
			data = ws.getRow(rownum).getCell(colnum).getStringCellValue();
			return data;
...
//		wb.close();
	...
	public static String[][] sheetData() throws Exception{
	    ...
	    String[][]empData = new String[rowCount][celCount];
	    
	    for(int i=1; i<= rowCount; i++) {
	    	for(int j=0; j<celCount; j++) {
	    		empData[i-1][j] = getCellData(path, constants.SheetNames.SHEET_NAME, i, j);
	    	}
	    }
